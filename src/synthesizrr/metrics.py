        ...
        *,
        ...
        if label_text == LABEL_OVERALL:
            if label_verbalizer != dataset_name.label_verbalizer():
                label_verbalizer_str: str = f"-vbs={String.hash(label_verbalizer, max_len=4)}"
        else:
            ...
            if label_verbalizer[label_text] != dataset_name.label_verbalizer()[label_text]:
                label_verbalizer_str: str = f"-vb={String.hash(label_verbalizer[label_text], max_len=4)}"

        text_gens_parser_str: str = ""
        text_gens_parser_body_hash: str = String.hash(get_fn_spec(text_gens_parser).source_body)
        if text_gens_parser_body_hash != String.hash(
        ...
        if save_type == "metrics":
            try:
                ...
                return (
                    ...
                    .file_in_dir(
                        ...
                        f"-metrics_num_samples_per_label={metrics_num_samples_per_label}"
                        f"-metric_name={metric_name.canonical()}"
                        f"-params_hash={String.hash(_params, max_len=6)}"
                        f".pkl",
                        return_metadata=True,
                    )
                    .update_params(file_format=FileFormat.PICKLE)
            ...
            try:
                ...
                return results_path.subdir_in_dir(
                    f"synthesizrr-generations-{save_type}-params_hash={String.hash(_params, max_len=6)}",
                    return_metadata=True,
                )
