...
import orjson
import pandas as pd
from bears import FileMetadata, Reader, to_sdf
...
def create_amazon_products():
    ...
    for df_part_i, df_part in enumerate(all_dfs):
        ...
        dest: str = corpus_split_dir.file_in_dir(
            f"amazon-reviews-2018-meta-part-{String.pad_zeros(df_part_i)}.parquet"
        )
        futs.append(
            run_concurrent(
                df_part.to_parquet,
    ...
    with Timer():
        ...
        for prods_part_i, prods_part in ProgressBar.iter(
            ...
            fpath: str = corpus_raw_text_dir.file_in_dir(
                f"amazon-products-2018-raw-text-part-{String.pad_zeros(prods_part_i)}.parquet"
            )
    ...
    with Timer(f"Writing partition {rn_name}"):
        ...
        for rn_part_i, rn_part in ProgressBar.iter(
            ...
            fpath: str = corpus_partition_dir.file_in_dir(
                f"realnews-{rn_name}-part-{String.pad_zeros(rn_part_i)}.parquet"
            )
...
def create_realnews():
    ...
    futs = []
    for df_part_i, df_part in enumerate(all_dfs):
        dest: str = corpus_split_dir.file_in_dir(f"realnews-part-{String.pad_zeros(df_part_i)}.parquet")
        futs.append(
            run_concurrent(
                df_part.to_parquet,
                dest,
