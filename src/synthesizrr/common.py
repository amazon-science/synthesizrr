...
import pandas as pd
import ray
from bears import FileMetadata, Reader, Writer
...
def get_templates_and_hashes(
    *,
    ...
    if icl_template is not None:
        icl_template_hash: str = String.hash(punct_normalize(icl_template), max_len=6)
    else:
    ...
    if prompt_template is not None:
        prompt_template_hash: str = String.hash(punct_normalize(prompt_template), max_len=6)
    else:
